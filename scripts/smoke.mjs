...
if (javascriptPath === undefined || cssPath === undefined) throw new Error("Web smoke check failed: Vite assets");
const [html, javascript, css] = await Promise.all([readFile("dist/index.html", "utf8"), readFile(javascriptPath, "utf8"), readFile(cssPath, "utf8")]);
const checks = [
...
for (const [passed, label] of checks) if (!passed) throw new Error(`Web smoke check failed: ${label}`);
const required = [...fixed, javascriptPath, cssPath];
const totalBytes = (await Promise.all(required.map(async (path) => (await stat(path)).size))).reduce((sum, size) => sum + size, 0);
